...
void draw(const char* filename, int* channels, int numChannels) {
    ...
    const char* names[3] = {"Front","Back", "Mid"}; 
    TGraph* graphs[numChannels];
    double maxValues[numChannels];
    for (int i = 0; i < numChannels; ++i) {
        TProfile* profile = dynamic_cast<TProfile*>(file->Get(TString::Format("AveragePulse_%s", names[i])));
 ...
 //   }
    ...
    for (int i = 0; i < numChannels; ++i) {
        if (graphs[i]) {
            ...
            if (i == 0) {
                graphs[i]->GetYaxis()->SetRangeUser(-1.2,0.3);
		graphs[i]->GetXaxis()->SetTitle("t-T_{trig} (ns)");
                graphs[i]->GetYaxis()->SetTitle("Amp (mV)");
                graphs[i]->SetTitle("Norm Averaged Channels FVB");
                graphs[i]->SetStats(0);
		graphs[i]->Scale(1/maxValues[i]);
		graphs[i]->SetFillColorAlpha(colors[i], 0.35);
                graphs[i]->SetFillStyle(3001);
                graphs[i]->Draw("bAL");
            } else {
		graphs[i]->Scale(1/maxValues[i]);
		graphs[i]->SetFillColorAlpha(colors[i], 0.35);
                graphs[i]->SetFillStyle(3001);
                graphs[i]->Draw("bL SAME");
            }
//graphs[i]->SetFillColorAlpha(colors[i], 0.35);
  //          graphs[i]->SetFillStyle(3001);
    ...
    line->SetPoint(0, 0, 0);
    line->SetPoint(1, 1020, 0);
    line->SetLineColor(kBlack);
    line->SetLineStyle(2);
    line->Draw("L SAME");
    TString canvasName = TString::Format("%s_NormChAvgPulse.png", filename);
...
int main() {
    ...
    int channels[] = {0,1,2};  //Repurposed this array to instead look at the Front and then the Back channels (that have been averaged together)
