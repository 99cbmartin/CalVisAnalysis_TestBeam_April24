...
void compareFilesForChannel(const char* file1, const char* file2, int channel, const char* outputDir, const char* sourceDir, Color_t color1, Color_t color2){
    ...
    TString profName1 = TString::Format("R4AveragePulseGraph_Ch%d", channel);
    TString profName2 = TString::Format("R4AveragePulseGraph_Ch%d", channel);

    TGraph* graph1 = dynamic_cast<TGraph*>(f1->Get(profName1));
    TGraph* graph2 = dynamic_cast<TGraph*>(f2->Get(profName2));
   ...
   // TGraph* graph1 = profileToGraph(prof1);
    ...
    TCanvas* canvas = new TCanvas(TString::Format("R4AvgPulseFVU_%s_Channel%d", file1, channel),
                                  TString::Format("R4 Average Pulse Channel %d: %s vs %s", channel, file1, file2), 800, 600);
	...
	graph1->GetXaxis()->SetTitle("t-T_{trig} (ns)");
        ...
        graph1->SetTitle(Form("R4 Average Pulse Filtered Vs. UnFiltered Channel %d",channel));
        graph1->SetStats(0);
	...
	graph1->Draw("AL");
        ...
        graph2->SetTitle(Form("R4 Average Pulse Filtered Vs. UnFiltered Channel %d",channel));
        graph2->SetStats(0);
    ...
    TString canvasName = TString::Format("%s/R4Norm_AvgPulseFVU_%s_Channel%d.png", outputDir, file1, channel);
...
int main() {
    const char* files1[] = {"run582_LG", "run594_LG"};
    const char* files2[] = {"run290_LG", "run284_LG"};
    const char* outputDir = "testbeam_plots/July/week4/AvgPulses";
    const char* sourceDir = "testbeam_plots/July/week4/";
    Color_t col[4] ={kGreen,kRed,kMagenta,kBlue}; 
    ensureDirectoryExists(outputDir);

    for (int channel = 0; channel < 8; ++channel) {
       // compareFilesForChannel(files1[0], files2[0], channel, outputDir, sourceDir,col[0],col[1]);
        //compareFilesForChannel(files1[1], files2[1], channel, outputDir, sourceDir,col[2],col[3]);
	compareFilesForChannel (files1[1],files2[1],channel,outputDir,sourceDir,col[1],col[3]);
    }
